...
Manifest-listed server files are staged atomically every 10 seconds. Open workspaces poll that staged revision and automatically apply changed entrypoint or `managed: true` text through the collaboration server—ordinary manuscript edits do not require an application deployment, browser reload, or manual `version` bump. Connected web edits to those managed text files are also written back atomically to the configured project source after a short debounce, so local tools and later Codex work start from the same current manuscript. Each write uses a server-side compare-and-set baseline. The service retains a small, private history of common source versions and performs a three-way merge when a local tool and the browser both changed the manuscript: independent line edits are combined, and independent character ranges on the same line receive one bounded fallback merge. Actual overlaps activate the latest local/server source and preserve the divergent web manuscript under `paper/drafts/web-conflict-*` for review. The editor distinguishes collaboration delivery from confirmed source-file writeback. A stale staged intermediate revision cannot roll a newer writeback backward. Duplicate tabs are deduplicated by the shared Yjs document and runtime revision checks. Offline edits remain in Yjs/IndexedDB and write back only after they reconnect and merge.

If a manifest entry points to a file that has since been deleted, runtime staging omits that stale entry, records it in `runtime_warnings`, and continues publishing the remaining current sources. A genuinely referenced missing file still fails LaTeX compilation with its normal missing-file error, but it can no longer freeze every unrelated source at the last successful runtime revision.

Set `"locked": true` on a managed file when it must remain server-authoritative. The editor makes it read-only, the collaboration server rejects stale-client writes and restores the disk value, and a divergent value found while enabling the lock is preserved under `paper/drafts/locked-web-edit-*`. This application lock does not prevent a local shell process from writing the file; use an operating-system immutable attribute as an additional project-specific guard when that is required.
