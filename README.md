...
> **현재 범위:** 문서는 브라우저 `localStorage`에 저장됩니다. WebSocket은 접속 상태와 커서만 공유하며 텍스트를 병합하거나 서버에 revision을 보관하지 않습니다. 계정·권한 관리도 아직 없습니다. 중요한 원고는 반드시 별도로 백업하세요.
...
```text
apps/paper_workspace/        UI, compiler, collaboration, Codex bridge
...
python -m py_compile apps/paper_workspace/compiler/server.py apps/paper_workspace/collaboration/server.py
docker compose -f infra/paper-workspace/compose.yaml config --quiet
```

기능 주장은 코드와 테스트로 확인 가능한 범위만 문서화합니다. 향후 과제는 인증/ACL, durable server storage, CRDT/OT 텍스트 동기화, revision history, compiler job queue와 quota입니다.
