...
function undoEditor(){if(isLockedProjectFile())return;const stack=historyFor(undoHistory,state.current);if(!stack.length)return;historyFor(redoHistory,state.current).push(editorSnapshot());applyEditorSnapshot(stack.pop())}
function redoEditor(){if(isLockedProjectFile())return;const stack=historyFor(redoHistory,state.current);if(!stack.length)return;historyFor(undoHistory,state.current).push(editorSnapshot());applyEditorSnapshot(stack.pop())}
...
function applyCodexRevision(selection,replacement){if(isLockedProjectFile()){alert('잠긴 파일은 수정할 수 없습니다.');return}if(selection.file!==state.current||editorValue().slice(selection.start,selection.end)!==selection.text){alert('Codex 요청 이후 원문이 변경되어 자동 적용하지 않았습니다. 다시 선택해 주세요.');return}historyFor(undoHistory,state.current).push(editorSnapshot());historyFor(redoHistory,state.current).length=0;richEditor.replaceRange(replacement,selection.start,selection.end,true);activeSelection={file:state.current,start:selection.start,end:selection.start+replacement.length,text:replacement};updateEditorMetadata();markProjectActivity('codex');hideSelectionToolbar();const apply=$('suggestion').querySelector('#apply-codex');if(apply){apply.textContent='✓ 원문에 적용됨';apply.disabled=true}}
...
async function loadProject(){
  ...
  for(const [path,text] of collabSession.files){if(manifestAssetPaths.has(path)||(projectVersionChanged&&serverManagedProjectFiles.has(path)))continue;const value=text?.toString?.();if(typeof value==='string'&&value)state.files[path]=value}
  pruneDraftQueue();
