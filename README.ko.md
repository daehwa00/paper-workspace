...
경로는 상대 경로여야 하며 `..`를 사용할 수 없습니다. 선택한 `.tex`가 독립 문서이면 직접 컴파일하고, appendix 같은 fragment이면 메인 preamble을 재사용합니다. 컴파일 한도는 파일 240개, 요청 48 MB, binary asset 합계 32 MB이고 브라우저 업로드는 파일당 8 MB입니다.
