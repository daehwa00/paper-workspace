...
매니페스트에 등록한 서버 파일은 10초마다 원자적으로 스테이징됩니다. 열린 작업공간은 새 리비전을 감지하고 entrypoint 또는 `managed: true` 텍스트를 협업 서버를 통해 자동 반영합니다. 연결된 웹에서 이 관리 텍스트를 수정하면 짧은 지연 뒤 설정된 프로젝트 원본에도 원자적으로 역반영되므로, 로컬 도구와 이후 Codex 작업도 같은 최신 원고에서 시작합니다. 역반영은 서버가 보관한 기준 해시와 실제 파일이 일치할 때만 수행합니다. 협업 서버는 최근 공통 원본을 제한된 비공개 이력으로 보존하고, 웹과 로컬이 함께 바뀌면 3-way 병합합니다. 서로 다른 줄의 변경을 먼저 합치고, 같은 줄에서도 서로 떨어진 단일 문자 범위라면 제한된 보조 병합을 한 번 수행합니다. 실제 변경 영역이 겹치면 웹 원고를 활성 상태로 유지하면서 로컬 변경안을 `paper/drafts/server-conflict-*`에 보존합니다. 편집기에는 공동 편집 전송과 실제 원본 파일 반영 완료가 구분되어 표시됩니다. 스테이징된 중간 리비전이 늦게 도착해도 더 최신인 웹 역반영본으로 되돌리지 않습니다. 오프라인 편집은 Yjs/IndexedDB에 남았다가 재연결·병합된 뒤 원본에 반영됩니다.
