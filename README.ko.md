...
매니페스트에 등록한 서버 파일은 10초마다 원자적으로 스테이징됩니다. 열린 작업공간은 새 리비전을 감지하고 entrypoint 또는 `managed: true` 텍스트를 협업 서버를 통해 자동 반영합니다. 일반 원고 수정에는 애플리케이션 재배포, 브라우저 새로고침, 수동 `version` 증가가 필요하지 않습니다. 연결된 웹 편집본이 이전 서버 스냅샷과 다르면 새 서버 원문을 적용하기 전에 `paper/drafts/`에 보존하며, 여러 탭의 중복 요청은 서버의 리비전 비교로 한 번만 적용됩니다. 아직 서버에 도달하지 않은 오프라인 Yjs 편집은 버리지 않고 재연결 때 병합하므로, 적용 전 별도 초안으로 보존된다고 보장되지는 않습니다.
